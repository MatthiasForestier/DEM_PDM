...
template <typename T>
T outerProduct(const VectorXF &a, const VectorXF &b, F mul = 1.0);

template <>
HessianF outerProduct<HessianF>(const VectorXF &a, const VectorXF &b, F mul) {
    return HessianF::outerProduct(a, b, mul);
}

template <>
MatrixXF outerProduct<MatrixXF>(const VectorXF &a, const VectorXF &b, F mul) {
    return mul * a * b.transpose();
}
...
T multiply(const T &a, const T &b, int order) {
    ...
    if (order >= 2)
        result.hessian = a.hessian * b.value + outerProduct<decltype(a.hessian)>(a.gradient, b.gradient) +
                         outerProduct<decltype(a.hessian)>(b.gradient, a.gradient) + b.hessian * a.value;
    return result;
}
...
T divide(const T &a, const T &b, int order) {
    ...
    if (order >= 2)
        result.hessian = a.hessian / b.value -
                         outerProduct<decltype(a.hessian)>(a.gradient, b.gradient, 1.0 / pow(b.value, 2)) -
                         outerProduct<decltype(a.hessian)>(b.gradient, a.gradient, 1.0 / pow(b.value, 2)) +
                         outerProduct<decltype(a.hessian)>(b.gradient, b.gradient, 2 * a.value / pow(b.value, 3)) -
...
T divide(const F &a, const T &b, int order) {
    ...
    if (order >= 2)
        result.hessian = outerProduct<decltype(b.hessian)>(b.gradient, b.gradient, 2 * a / pow(b.value, 3)) -
                         b.hessian * a / pow(b.value, 2);
    return result;
}
...
T powf(const T &a, const F &b, int order) {
    ...
    if (order >= 2)
        result.hessian =
            outerProduct<decltype(a.hessian)>(a.gradient, a.gradient, pow(a.value, b - 2.0) * b * (b - 1.0)) +
